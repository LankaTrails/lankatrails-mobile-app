...
const ServiceView = () => {
  ...
  useEffect(() => {
    (
      Object.keys(progressAnimations) as Array<keyof typeof progressAnimations>
    ).forEach((key) => {
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      <ScrollView className="bg-gray-50">
        <ScrollView horizontal className="ml-6 mt-4">
          {serviceImages.map((img, i) => (
            <Image
              key={i}
              source={{ uri: img }}
              className="w-96 h-96 rounded-lg mr-4"
            />
          ))}
        </ScrollView>
        ...
        <View className="px-4 mt-4">
          ...
          <Text className="text-lg text-gray-500 mt-1">
            Large grilled prawns served with garlic butter and herbs.
          </Text>
        ...
        <View className="flex-row space-x-2 px-4 gap-2 mt-2">
          ...
          <Text className="text-xs px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full">
            Spicy
          </Text>
        ...
        <View className="px-4 mt-4">
          ...
          <Text className="text-gray-600">
            🦐 King prawns, 🧄 Garlic butter, 🌿 Local herbs, 🧂 Sea salt
          </Text>
        </View>

        <View className="px-4 mt-4">
          <Text className="text-lg italic text-gray-700">
            "Our grilled prawns are prepared using a 50-year-old recipe passed
            down from the original beach chefs of Unawatuna."
          </Text>
        </View>
      ...
      <Modal visible={showTripModal} transparent animationType="none">
        ...
        >
          ...
          >
            {/* <TouchableOpacity
              onPress={() => setTripType('existing')}
              style={{
                paddingHorizontal: 16,
                paddingVertical: 8,
                borderRadius: 999,
                backgroundColor: tripType === 'existing' ? '#008080' : '#e5e7eb',
              }}
            >
              <Text style={{ color: tripType === 'existing' ? '#fff' : '#374151', fontWeight: '600' }}>Existing Trip</Text>
            </TouchableOpacity> */}
            {/* <TouchableOpacity
              onPress={() => setTripType('new')}
              style={{
                paddingHorizontal: 16,
                paddingVertical: 8,
                borderRadius: 999,
                backgroundColor: tripType === 'new' ? '#008080' : '#e5e7eb',
              }}
            >
              <Text style={{ color: tripType === 'new' ? '#fff' : '#374151', fontWeight: '600' }}>New Trip</Text>
            </TouchableOpacity> */}
          </View>
