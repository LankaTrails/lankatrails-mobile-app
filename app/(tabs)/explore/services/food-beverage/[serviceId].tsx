...
import {
  fetchFoodBeverageService,
  isFoodBeverageService,
...
// Convert FoodBeverageServiceDetail to Service for AddToTripButton
const convertToService = (detail: FoodBeverageServiceDetail): Service => ({
...
const FoodBeverageServiceDetailPage = () => {
  ...
  const [serviceDetail, setServiceDetail] =
    useState<FoodBeverageServiceDetail | null>(null);
  ...
  useEffect(() => {
    const fetchServiceDetails = async () => {
      ...
      try {
        ...
        const response: ApiResponse<FoodBeverageServiceDetail> =
          await fetchFoodBeverageService(parseInt(serviceId));
    ...
    <>
      ...
      <ScrollView className="flex-1 mb-20 bg-gray-50">
        ...
        <View className="px-4 py-3 border-t border-b border-gray-100 bg-white">
          ...
          {/* Opening Hours */}
          <View className="flex-row items-center mt-3">
            <Clock size={24} color="#008080" />
            <Text className="ml-4 text-gray-700 text-lg">
              {serviceDetail.openHours}
            </Text>
          </View>
        </View>
          ...
          <View className="bg-white rounded-lg p-4 mb-4 shadow-sm">
            <View className="flex-row justify-between mb-3">
              ...
              <Text className="text-gray-600 capitalize">
                {serviceDetail.foodAndBeverageType
                  .toLowerCase()
                  .replace("_", " ")}
              </Text>
            </View>
            <View className="flex-row justify-between">
              <Text className="font-semibold text-gray-700">Cuisine:</Text>
              <Text className="text-gray-600">{serviceDetail.cuisineType}</Text>
            </View>
          </View>
        </View>
          ...
          <View className="bg-white rounded-lg p-4 shadow-sm">
            <View className="flex-row flex-wrap">
              {renderFeature(
                "leaf",
                "Vegetarian Options",
                serviceDetail.vegetarianOptions
              )}
              {renderFeature(
                "restaurant",
                "Halal Certified",
                serviceDetail.halalCertified
              )}
              {renderFeature(
                "wine",
                "Alcohol Served",
                serviceDetail.alcoholServed
              )}
              {renderFeature(
                "cafe",
                "Outdoor Seating",
                serviceDetail.outdoorSeating
              )}
              {renderFeature(
                "musical-notes",
                "Live Music",
                serviceDetail.liveMusic
              )}
            </View>
          </View>
        </View>
...
export default FoodBeverageServiceDetailPage;
